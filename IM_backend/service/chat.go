...
func WsHandler(c *gin.Context) {
	...
	conn, err := (&websocket.Upgrader{}).Upgrade(c.Writer, c.Request, nil) // 升级成ws协议
...
func (c *Client) Read() {
	...
	for {
		...
		if sendMsg.Type == 1 {
			...
			results, _ := FindMany(conf.MnongoDBName, c.SendID, c.ID, int64(timeT), 10)
			if len(results) > 10 {
				results = results[:10]
			...
			results, err := FirsFindtMsg(conf.MnongoDBName, c.SendID, c.ID)
			if err != nil {
				log.Println(err)
			}
			for _, result := range results {
